...
eurovision_temp <- merge(x = eurovision_temp, y = eurovision %>% select(distance_hav, To_country, From_country), by = c("To_country", "From_country"), all.x = TRUE) %>% distinct
...
set.seed(20)
clusters <- kmeans(standardized_euro[,8:11], 6)
...
ggnet2(my_net, label = TRUE, arrow.size = 6, arrow.gap = 0.025, edge.size = as.vector(eurovision_temp1$edge_weight),  palette =  c("1" == "Pink", "2" == "Blue", "3" == "Green", "4" == "Plum", "5" == "Grey"))
